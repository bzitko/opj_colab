...
SENTIMENT_FILENAME = './data/sentiment_lexicon.csv'
...
def read_sentiments(filename):
    ...
    izlaz:
    -rječnik oblika w: klasa gdje je w riječ,
     a klasa 1 (za positive) ili 0 (za negative)
    """
    sentiments = {}
    with open(filename) as fp:
        for line in fp:
            w, k = line.rstrip().split(",")
            sentiments[w.lower()] = 1 if k == "pos" else 0
    return sentiments
...
def read_document(filename, stopwords=set()):
    ...
    izlaz:
    -lista riječi pretvorenih u mala slova iz datoteke koje nisu u stopwords
    """
    txt = open(filename, 'r').read()
    return [word.lower() for word in txt.split() if word and word.strip() not in stopwords]


def make_features(document, sentiments):
    """
    ulaz:
    -document: lista riječi
    -sentiments: rječnik sentimenata
    izlaz:
    -lista brojeva oblika
        [broj pozitivnih riječi,
         broj negativnih riječi,
         broj negacija: no, not ili none,
         broj zamjenica: i, you, me,
         broj uskličnika,
         prirodni logaritam ukupnog broja riječi]
    """
    num_positive = 0
    num_negative = 0
    num_negation = 0
...
def make_datasets(data_path, sentiments, numfolds=10):
    """
    ulaz:
    ...
    čije su vrijednosti rječnik s dva ključa "1" za pos i "0" za neg čije su
    vrijednosti lista osobina datoteka koje su "1" ili "0" kritike
    """
    klasses = [1 if klass == "pos" else 0
               for klass in os.listdir(data_path)
               if not klass.startswith('.')]
    datasets = []
    filenames = {klass: sorted(os.listdir(data_path + ("pos" if klass == 1 else "neg") + '/')) for klass in klasses}
    print("Reading fold:", end=" ")
    for fold in range(numfolds):
        ...
        for klass in klasses:
            for filename in filenames[klass]:
                doc = read_document(data_path + ("pos" if klass == 1 else "neg") + '/' + filename)
                x = make_features(doc, sentiments)
                if filename[2] == str(fold):
                    tests.append((x, klass))
                else:
                    trains.append((x, klass))
        datasets.append({'train': trains, 'test': tests})
    print()
    return datasets


datasets = make_datasets(DATA_PATH, read_sentiments(SENTIMENT_FILENAME), numfolds=1)
...
def all(trainset, testset, n_iter=500, rate=0.01):
    m, n = len(trainset), len(trainset[0][0])
    ...
    for i in range(n_iter):
        ...
        X = np.array([xi for xi, _ in trainset])  # m x n
        X1 = np.c_[X, np.ones(X.shape[0])]  # m x (n + 1)
        Y = np.array([yi for _, yi in trainset])  # m x 1

        FX = sigmoid(np.matmul(Wb, X1.T))  # m x 1
        assert FX.shape == (m,)

        GRAD = np.matmul(X1.T, FX - Y) * (1 / m)
        assert GRAD.shape == (n + 1,)
    ...
    XT = np.array([xi for xi, _ in testset])
    X1T = np.c_[XT, np.ones(XT.shape[0])]
    YT = np.array([yi for _, yi in testset])

    PREDICT = sigmoid(np.matmul(Wb, X1T.T))
    PREDICT = np.where(PREDICT > 0.5, 1, 0)

    num_errors = np.count_nonzero(PREDICT - YT)
    num_all, _ = XT.shape
    acc = (num_all - num_errors) / num_all
    print(f"Acc: {acc}")
...
w, b = training(datasets[0]["train"], num_iters=2000, learn_rate=0.0001, batch_size=512)
acc = testing(datasets[0]["test"], w, b)
print(acc)
exit()
...
class NBClassifier(object):
    ...
    def add_train_doc(self, klass, filename):
        ...
        """
        self.documents[klass] += 1
        for word in read_document(filename, self.stopwords):
            self.likelihood[klass][word] = self.likelihood[klass].get(word, 0) + 1
    ...
    def train(self):
        ...
        """
        self.prior = {klass: count / sum(self.documents.values()) for klass, count in self.documents.items()}
        all_likelihood_size = len(self.vocabulary)
        self.denominator = {klass: sum(vocab.values()) + all_likelihood_size for klass, vocab in self.likelihood.items()}
    ...
    def classify(self, filename):
        ...
        """
        words = read_document(filename, self.stopwords)

        posterior = {}
        for klass in self.klasses:
            posterior[klass] = math.log(self.prior[klass])
            for word in words:
                posterior[klass] += math.log(self.likelihood[klass].get(word, 0) + 1) - math.log(self.denominator[klass])

        klass = max(posterior, key = posterior.get)
        return klass
